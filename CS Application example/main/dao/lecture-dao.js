...
/*  select l.titl, l.dscp, date_format(l.sdt,'%y-%m-%d') sdt, l.edt, l.qty, l.pric, l.thrs,
  ...
  */
  selectOne(no, successFn, errorFn) {
    console.log('ddds')
    this.connection.query(
      "select l.titl, l.dscp, date_format(l.sdt,'%Y-%m-%d') sdt, date_format(l.edt,'%Y-%m-%d') edt, l.qty, l.pric, l.thrs, \
      c.name cname, m.name mname, l.crmno, l.mrno\
      from lect l \
      left join croom c on l.crmno=c.crmno \
      left join mgr mr on l.mrno=mr.mrno \
...
*/
  update(lecture, successFn, errorFn) {
    this.connection.query(
      ...
      where lno=?",
      [lecture.titl, lecture.dscp, lecture.sdt, lecture.edt,
      lecture.qty, lecture.pric, lecture.thrs, lecture.crmno, lecture.mrno, lecture.no],
      function (error, results) {
        if (error) {
          errorFn(error)
        } else {
...
*/
  insert(lecture, successFn, errorFn) {
    this.connection.query(
      "insert into lect (titl, dscp, sdt, edt, qty, pric, thrs, crmno, mrno) \
      values (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [lecture.titl, lecture.dscp, lecture.sdt, lecture.edt, lecture.qty,
      lecture.pric, lecture.thrs, lecture.crmno, lecture.mrno],
      function(error, results) {
        if (error) {
          errorFn(error)
        } else {
